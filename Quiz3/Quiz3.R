## 1. In a population of interest, a sample of 9 men yielded a sample average 
...
print(1100+c(-1,1)*qt(0.975,8)*30/sqrt(9))
...
print(2*sqrt(9)/qt(0.975,8))

## 3. In an effort to improve running performance, 5 runners were either given 
## a protein supplement or placebo. Then, after a suitable washout period, 
## they were given the opposite treatment. Their mile times were recorded under 
## both the treatment and placebo, yielding 10 measurements with 2 per subject.
...
sp<-sqrt((9*0.60+9*0.68)/18)
print(-2+c(-1,1)*qt(0.975,18)*sp*sqrt(1/10.+1/10.))

## 5. Suppose that you create a 95% T confidence interval. You then create a 
## 90% interval using the same data. What can be said about the 90% interval 
## with respect to the 95% interval?
...
## of 2 hours. Consider the hypothesis of a decrease in the mean MWT associated 
...
print(2+c(-1,1)*qt(0.975,198)*sp*sqrt(1/100.+1/100.))

## Answer : When subtracting (old-new) the interval is entirely above zero.
...
print(-4+c(-1,1)*qt(0.95,16)*sp*sqrt(1/9.+1/9.))