...
## μ0 that a test of H0:μ=μ0 would fail to reject the null hypothesis in
## a two sided 5% Students t-test?
...
print(binom.test(3,4,alt="greater")$p.value)
...
## infections over the last 1,787 person days at risk. About what is the one
## sided P-value for the relevant test of whether the hospital is *below* 
## the standard?

print(poisson.test(x=10,T=1787,r=1/100,alt="less")$p.value)

## 5. Suppose that 18 obese subjects were randomized, 9 each, to a new diet 
...
sp<-(8*1.8^2+8*1.5^2)/16
print(pt((-3-1)/(sp*(1/9+1/9)^0.5),df=16))
...
## standard deviation of four year volume loss in this population is .04 mm3.
## About what would be the power of the study for a 5% one sided test versus 
## a null hypothesis of no volume loss?

print(power.t.test(n=100,delta=0.01,sd=0.04,sig.level = 0.05, type="one.sample",alt="one.sided")$power)

## 8. Researchers would like to conduct a study of n healthy adults to 
## detect a four year mean brain volume loss of .01 mm3. Assume that the 
## standard deviation of four year volume loss in this population is .04 mm3.
## About what would be the value of n needed for 90% power of type one error
## rate of 5% one sided test versus a null hypothesis of no volume loss?

print(power.t.test(power=0.9,delta=0.01,sd=0.04,sig.level = 0.05, type="one.sample",alt="one.sided")$n)
